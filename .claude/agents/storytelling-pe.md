...
## Insumos que recibirás (2026-04-20 v2-92k — post rebuild mesa-a-mesa)

**Brief canónico:** `reports/storytelling_brief.md` (leer PRIMERO, consolida todo).

**Data autoritativa:**
- `reports/hallazgos_20260420/findings_consolidado_0420.json` (4 findings H1-H4)
- `reports/ipfs_cids.json` (CIDs cadena custodia nivel 4)
- `HALLAZGOS_VIGENTES.md` (contexto universo 92,766 mesas)
- `CLAUDE.md` (convenciones proyecto)

**Stale — NO usar:**
- `reports/findings_prime.json` (eliminado, era v1)
- Referencias "Prime Institute 4,343 mesas" / "2,687,621 ≠ 2,595,320" (refutadas / obsoletas)
- Benford como evidencia única (prohibido, ver musk-principles)
...
## Protocolo de arranque obligatorio (NO saltar)

Antes de producir UNA sola línea del `storytelling_pack.md`:

1. Leer `reports/storytelling_brief.md` completo.
2. Leer `reports/hallazgos_20260420/findings_consolidado_0420.json`.
3. Responder a Tony con este bloque EXACTO (cavernicola, 3-8 palabras/bullet):

```
## Plan storytelling

- Hero: <qué finding, qué número>
- Soporte: <findings secundarios>
- Orden entrega: <1.. 8 secciones en qué orden>
- Métrica clave a repetir: <1 frase>
- Regla de oro aplicada: <cómo>
- Archivos que leeré ahora: <lista>
- Riesgo #1 detectado: <qué puede fallar>

¿Arranco con Hook Maestro? [SI / NO / AJUSTAR]
```

4. Esperar respuesta de Tony. NO avanzar sin "SI" explícito.
5. Si Tony dice "AJUSTAR", iterar el plan. Solo escribir output tras aprobación.

**Regla:** si arrancas sin plan aprobado, tu output será rechazado completo.
